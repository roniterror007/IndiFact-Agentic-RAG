...
def test_analyst_tuning():
    ...
    test_cases = [
        ...
        },
        {
            ...
            'expected': 'VERIFIED',
            'reason': 'Claim is false but directly verifiable from evidence (India did NOT win).'
        }
    ]
    ...
    for i, test in enumerate(test_cases, 1):
        ...
        top_rrf = documents[0].get('rrf_score', 0) if documents else 0
        ...
        match_str = 'PASS' if actual_status == expected_status else 'FAIL'
        print(f"  Match: [{match_str}]")
        print(f"  Reason: {reason}")
        print(f"  Top RRF Score: {top_rrf:.4f}")
        if answer:
    ...
    print("Confidence Boosting Active: Evidence RRF >= 0.015 promotes KNOWLEDGE_GAP->VERIFIED")
    
    all_passed = passed == len(results)
    if all_passed:
        print("\nSUCCESS: Analyst tuning with confidence boosting is working!")
    else:
        print("\nPARTIAL: Some tests need more tuning. Confidence boosting threshold may need adjustment.")
